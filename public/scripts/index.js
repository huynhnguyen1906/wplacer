...
function getFilteredLogs() {
    ...
    if (currentFilterType) {
        filtered = filtered.filter((line) => {
            ...
            if (currentFilterType === 'success')
                return /success|started|running|ok|ready|listening|connected/i.test(line);
    ...
    if (templateId) {
        try {
            ...
            colorEntries = templateColors
                .map((c) => [
                    Object.keys(colors).find((rgb) => colors[rgb].id === c.id),
                    colors[Object.keys(colors).find((rgb) => colors[rgb].id === c.id)],
                ])
                .filter(([rgb]) => rgb);
        } catch {
            availableColors = new Set(Object.values(colors).map((c) => c.id));
...
async function buildGrid(colorEntries, templateId = null) {
    try {
        const { order = [] } = await (
            await fetch(templateId ? `/color-ordering?templateId=${templateId}` : `/color-ordering`)
        ).json();
...
const resetOrder = () =>
    buildGrid(Object.entries(colors).filter(([_, data]) => !currentTemplateId || availableColors.has(data.id)));
const updateColorGridForImage = (imageColorIds) => {
    availableColors = new Set(imageColorIds);
    const imageColors = imageColorIds
        .map((id) => [
            Object.keys(colors).find((rgb) => colors[rgb].id === id),
            colors[Object.keys(colors).find((rgb) => colors[rgb].id === id)],
        ])
        .filter(([rgb]) => rgb);
